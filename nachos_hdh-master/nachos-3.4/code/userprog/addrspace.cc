...
AddrSpace::AddrSpace(OpenFile *executable)
{
    ...
    if (executable == NULL){
        printf("Addrspace::Error opening file.\n");
        return;
    }
...
// first, set up the translation 
	...
	pageTable[i].physicalPage = gPhysPageBitMap->Find(); // tim o trong de lap cho
...
AddrSpace::AddrSpace(char * filename) {
    ...
    unsigned int numCodePage, numDataPage; // số trang cho phần code và phần initData int lastCodePageSize, lastDataPageSize, firstDataPageSize,tempDataSize; // kích
    //thước ghi vào trang cuối Code, initData, và trang đầu của initData
    OpenFile* executable = fileSystem->Open(filename);
    if (executable == NULL){
        printf("\nAddrspace::Error opening file: %s",filename); DEBUG(dbgFile,"\n Error opening file.");
        return;
    }
    //đọc header của file
    executable->ReadAt((char *)&noffH, sizeof(noffH), 0); if ((noffH.noffMagic != NOFFMAGIC) &&
